...
function storePhotos(album_name, album_id, coverphotoid, all_albums){
    FB.api(album_id + '/photos', function(response){
        ...
        return returnPhotoLINKS(response.data).then(function(all_photos){
            console.log("All Photos"+all_photos)
            return all_photos
        }).then(function(x){
            coverphotolink = returnPhotoLINK(coverphotoid);
            console.log("Cover Photo Link" + coverphotolink);
            return [x, coverphotoid];
        }).then(function(container_object){
            all_albums.push({
            'name': album_name,
            'cover_photo': container_object[1],
            'photos': container_object[0]
            });
            console.log("All Albums"+all_albums);
            return all_albums;
        });
